package O_graph.A_adjacencyList;
...
public class AdjacencyListGraph {
  ...
  public void addUndirectedEge(int sourceIndex, int targetIndex) {
    var sourceNode =
        graph.stream().filter(graphNode -> graphNode.index == sourceIndex).findFirst().orElse(null);
    var targetNode =
        graph.stream().filter(graphNode -> graphNode.index == targetIndex).findFirst().orElse(null);
