...
public class DeepCopyOfLinkedList {
    ...
    ListWithRandom newList = new ListWithRandom();
    ListWithRandom list;

    public DeepCopyOfLinkedList(ListWithRandom list){
        this.list = list;
    }
    ...
    // Node temp = curr.next; --> curr.next = new Node(curr.val); --> curr.next.next = temp; --> curr=temp

    // Step 2: Now we will populate the new Node random using the old node. [curr = head]
    // curr.next.random = curr.random != null ? curr.random.next : null

    //Step 3: Now disconnecting the new list with old [original = head && originalCopy = head.next && newListHead = originalCopy]
    // orignal.next = original.next.next;
    // orignalCopy.next = originalCopy.next.next

    public ListWithRandom deepCopy(){
